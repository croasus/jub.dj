...
function JubDJ(config, gapi, chat, db) {
  ...
  this.stop_fns = [db.stop];             // List of functions to call when shutting down
      ...
      rotate_djs(function(next_dj) {
        ...
        if (next_dj) {
          rotate_selected_playlist(next_dj, function(next_video) {
            if (next_video) {
              jub.start_new_video(next_dj, next_video);
            }
  ...
  this.stop = function() {
    ...
    _.each(jub.stop_fns, function(fn) { fn(); });
  }
  ...
  this.get_chat_cache = function(callback) {
...
module.exports = function(config, gapi, chat, db) {
  ...
  if (process.env.TEST) { jub.add_dj(chat.bot.name); }
  return jub
}
