...
function JubDJ(config, gapi, chat, db) {
  ...
  this.setLocalKarmaMap = function(map) {
    console.log("Loading comprehensive user-karma map");
    jub.karmaMap = map;
  }

  this.refreshUsersForClients = function() {
    jub.broadcast('current users', jub.emittableUserMap());
  }

  this.giveKarmaAndGetTotal = function(k, callback, errCb) {
    if (currentUsers.has(k.recipient)) {
      db.addKarma(k, function() {
        db.getUserKarmaTotal(k.recipient, callback, errCb); //for acting on single user's karma
        db.getKarmaReceived(jub.setLocalKarmaMapAndRefreshUsers); //update local karma map
      }, errCb);
    }
  };
