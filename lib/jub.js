...
function JubDJ(config, gapi, chat, db, auth, mail) {
  ...
  this.setCurrentUsers = function(users, callback) {
    ...
    joined.forEach(function(user) {
      chat.botSay(user + ' has joined the pub.');
    })

    left.forEach(function(user) {
  ...
  this.login = function(username, password, expiration, callback) {
    db.validateLogin(username, password, function(isValid) {
      ...
      db.getFormattedName(username, function(formattedName, errMsg) {
    ...
    if (currentUsers.has(nickname)) {
      ...
      db.accountExists(nickname, function(taken) {
        ...
        if (success) {
          ...
          auth.createSessionToken(expiration, data, jub.secret, function(token) {
            //TODO currentUsers.add(nickname);
            callback(token, '');
          });
        } else {
  ...
  this.requestPasswordReset = function(username, callback) {
    db.requestPasswordReset(username, function(token, errMsg) {
      ...
      db.getEmailAddress(username, function(emailAddress, errMsg) {
        if (errMsg) {
