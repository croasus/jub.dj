...
function JubDJ(config, gapi, chat, db) {
  ...
  this.giveKarmaAndGetTotal = function(k, callback) {
  ...
  this.getKarma = function(user, callback) {
    db.getUserKarmaTotal(user, callback);
  };
  ...
  function newPlaylistName(user, baseName, callback) {
    db.fetchOrInitPlaylists(user, function(lists) {
      ...
      var num = 1;
      while (_.includes(existing, newName + ' ' + num)) { // TODO this is incorrect
        num = num + 1;
      }
  ...
  this.deletePlaylist = function(user, playlistName, whisperToClient) {
    if (playlistName != 'sandbox') {
      ...
      jub.updateUserPreferences(user, { selectedPlaylist: 'sandbox' });
    }
  }
  ...
  this.setCurrentUsers = function(users, callback) {
    ...
    // Not allowed to be in DJ rotation if not present
    _.remove(djSched, function(user) { return !currentUsers.has(user) });
    callback(jub.emittableUserMap());
  }
