...
function JubDJ(config, gapi, chat, db, auth, mail) {
  ...
  let currentUsers = new Set([chat.bot.name]); // users present in room
  ...
  let queueMap = function(callback, errCb) {
  ...
  let queueLenMap = function(callback, errCb) {
    queueMap(map => {
      let lenMap = _.reduce(map, (result, queue, user) => {
  ...
  // playlists
  let updateUpNext = function(release) {
  ...
  this.setCurrentUsers = function(users, callback) {
    let oldCurrentUsers = currentUsers.clone();
    currentUsers.clear();
    currentUsers.add(chat.bot.name);
    currentUsers.addEach(users);

    let joined = currentUsers.difference(oldCurrentUsers);
    let left = oldCurrentUsers.difference(currentUsers);
