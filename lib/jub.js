...
function JubDJ(config, gapi, chat, db, auth, mail) {
  ...
  this.giveKarmaAndGetTotal = function(k, callback, errCb) {
    if (currentUsers.has(k.recipient)) {
      db.accountExists(k.giver, function(exists) {
        db.accountExists(k.recipient, function(exists) {
  ...
  this.joinAsGuest = function(nickname, expiration, callback) {
    ...
    if (currentUsers.has(nickname)) {
      ...
      db.accountExists(nickname, function(taken) {
        if (taken) {
    ...
    db.requestPasswordReset(username, function(token, errMsg) {
      ...
      db.getEmailAddress(username, function(emailAddress, message) {
        ...
        var message = "<b>Click this link to reset your password:</b> " +
                      ...
                      '<a href="mailto:contact@thejub.pub?subject=' + errorSubject +
