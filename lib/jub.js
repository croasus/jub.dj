...
function JubDJ(config, gapi, chat, db, auth) {
  ...
  this.renamePlaylist = function(user, fromName, toName, errCb) {
    ...
    async.series([
      ...
      function(done) {
        ...
        if (fromName === 'sandbox') {
          jub.createPlaylist(user, 'sandbox', done, errCb);
        } else {
          done();
        }
      },
  ...
  this.copyPlaylist = function(user, playlistName, errCb) {
    ...
    async.series([
      ...
      function(done) {
        ...
        jub.createPlaylist(user, newName, function(playlist) {}, done, errCb);
      },
      function(done) {
  ...
  // TODO don't need force reload anymore
  ...
  /*
  this.userLoaded = function(user) {
    console.log('user loaded', user);

    // Don't need to force reload this user TODO remove usersSeen
    usersSeen.add(user);
  };
  */
  ...
  this.joinAsGuest = function(nickname, expiration, callback) {
    if (currentUsers.has(nickname)) {
      callback(null, "That nickname is already taken :-(");
    } else {
