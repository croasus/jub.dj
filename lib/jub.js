...
function JubDJ(config, gapi, chat, db, auth, mail) {
  ...
  var rotateSelectedPlaylist = function(user, callback, errCb) {
    ...
    jub.fetchPlaylistsFor(user, function(lists, selected) {
      async.series([
        function(done) {
          db.fetchUserPreferences(user, function(prefs) {
            userPrefs = prefs;
            done();
          }, errCb);
        },
  ...
  this.updatePlaylistMeta = function(user, playlistName, updateFn, doneFn, errCb) {
    ...
    async.series([
      // Fetch/create the playlist and call the update function
      function(done) {
        db.updatePlaylistMeta(user, playlistName, function(playlist, doneUpdating) {
          updateFn(playlist);
  ...
  this.updateUserPreferences = function(user, updates, done, errCb) {
    ...
    db.updateUserPreferences(user, updates, function(updated) {
  ...
  this.fetchPlaylistsFor = function(user, callback, errCb) {
    ...
    async.series([
      ...
      function(done) {
        db.fetchPlaylists(user, function(fetched) {
          ...
          console.log('fetched playlists for', user, emittable, selected);
  ...
  function newPlaylistName(user, fromName, callback, errCb) {
    db.fetchPlaylists(user, function(lists) {
  ...
  this.copyPlaylist = function(user, playlistName, errCb) {
    ...
    async.series([
      ...
      function(done) {
        ...
        db.createPlaylist(user, newName, function(playlist) {}, done, errCb);
      },
      function(done) {
  ...
  this.login = function(username, password, expiration, callback) {
    db.validateLogin(username, password, function(isValid) {
      if (isValid) {
        var data = 'account,' + username; // TODO use JWT
        auth.createSessionToken(expiration, data, jub.secret, function(token) {
          callback(token);
        });
      } else {
        callback(null);
  ...
  this.joinAsGuest = function(nickname, expiration, callback) {
    ...
    if (currentUsers.has(nickname)) {
      callback(null, "That name is already taken :-(");
    } else {
      // Check if any absent user has the name
      db.isUsernameTaken(nickname, function(taken) {
        if (taken) {
  ...
  this.signup = function(username, password, email, callback) {
    db.createAccount(username, password, email, function(errMsg) {
      if (errMsg) { return callback(errMsg); }
      db.initPlaylists(username, function() {
        callback(null);
      }, function(error) { // This is the error callback
  ...
  this.resetPassword = function(username, token, password, callback) {
    console.log('jub.resetPassword', username, token, password);
    db.validatePasswordResetToken(username, token,
                                  function(success, message) {
      console.log('resetPassword first callback', success, message);
      if (success) {
  ...
  this.requestPasswordReset = function(username, callback) {
    db.requestPasswordReset(username, function(token, errMsg) {
      db.getEmailAddress(username, function(emailAddress) {
        var recipient = emailAddress;
        var subject = "Reset your password";
        var resetLink = jub.generateResetPasswordLink(username, token);
        ...
        message += "If you did not initiate a password reset, ignore the " +
                   "link and please report the incident to ";
        message += '<a href="mailto:contact@thejub.pub?subject=' + errorSubject +
                   '">contact@thejub.pub</a>.';
