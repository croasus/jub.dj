...
require('./logging')();
var urban = require('../external_lib/urban');

function Bot(config, gapi, twitter) {
  this.name = "jubbot";
      ...
      if (msg.search(/^penis/) >= 0) {
        ...
        if (query.length > 1) {
          ...
          res.first(function(json) {
            var displayMsg = 'Your query, ' + query[1] +', doesn\'t seem to exist.';
            if( json && json.hasOwnProperty('definition')) {
              displayMsg = query[1] + ': ' + json.definition;
            }
            callback(bot_msg_obj(displayMsg));
          });
        }
      } else {
...
module.exports = function(config, gapi, twitter) {
  return new Bot(config, gapi, twitter);
}
