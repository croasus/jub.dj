...
require('./logging')();
var latest_updates = require('../latest_updates');
...
function Bot(config, gapi, twitter, urban) {
  ...
  this.welcome = function(user, callback) {
    ...
    if (latest_updates && latest_updates.list.length > 0) {
      msg += '\nLatest updates';
      if (latest_updates.date) {
        msg += ' (' + latest_updates.date + ')';
      }
      msg += ':\n' +
        latest_updates.list
  ...
  this.newChatMessage = function(msgObj, callback) {
    ...
    if (msg.startsWith(this.name + ':')) {
      ...
      var query;
      if (msg === 'help') {
        var resp = "Hi! I answer questions and keep you updated on what's \
        ...
        and check someone's karma with '!karma username'."
