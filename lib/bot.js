      ...
      if (msg.toLowerCase() === 'help') {
        ...
        if (query.length > 1) {
          ...
          var whitelistIPs = _.reduce(config.chat.whitelist, (a, user) => {
            var ips = bot.getUserIPs(user);
            _.each(ips, ip => { a[ip] = _(a[ip]).concat(user).uniq().value(); });
            return a;
          }, {});
          for (ip of ips) {
          ...
          console.log('blacklisted', user, ips);
          callback(botMsgObj('SEE YA, ' + user + '!!'));
          bot.requestReloadUser(user);
        }
        ...
        if (query.length > 1) {
          ...
          p.stderr.on('data', function(chunk) {
            callback(botMsgObj("Hmm, nah I can't do it."));
            console.error("markgen error: " + chunk.toString());
          });
          p.on('close', function(a, b) {
            console.error("markgen exited: ", a, b);
          });
          p.on('error', function(a, b) {
            callback(botMsgObj("I... can't :-("));
            console.error("markgen exited: ", a, b);
          });
        }
