...
function Report(db, gapi) {
  let REPORT_LIST_LIMIT = 15;

  let filterNull = function(obj, callback) {
  ...
  let reverseSortLimit = function(obj, callback) {
    obj = _(obj).toPairs()
  ...
  let videoTitles = function(obj, callback) {
    let videoObjs = _(obj).toPairs()
    ...
    async.reduce(reportSpec, [], (memo, sectionSpec, nextReduce) => {
      sectionSpec.fn(result => {
        ...
        async.waterfall(post, (err, result) => {
          try {
            let result = { title: sectionSpec.title, data: result };
            memo.push(result);
            nextReduce(null, memo);
          } catch(err) {
