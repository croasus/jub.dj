...
function Report(db, gapi) {
  ...
  var reverseSortLimit = function(obj, callback) {
    obj = _(obj).toPairs()
                .sortBy(function(v) { return sum(v[1]); })
                .reverse()
  ...
  var videoTitles = function(obj, callback) {
    var videoObjs = _(obj).toPairs()
                          .map(function(x) { return { id: x[0], hootCount: x[1] }; })
                          .value();
    async.reduce(videoObjs, {}, function(memo, videoObj, nextReduce) {
      gapi.videoSpecs(videoObj, function(result) {
  ...
  this.getKarmaReport = function(callback, errCb) {
    ...
    async.reduce(reportSpec, [], function(memo, sectionSpec, nextReduce) {
      sectionSpec.fn(function(result) {
        // Prepare the post-processing fns
        var post = sectionSpec.post || [];

        // Kick off the waterfall with the initial data
        post.unshift(function(cb) { cb(null, result); });

        async.waterfall(post, function(err, result) {
          try {
