...
function Socketeer(jub, chat, bot, config, io) {
  ...
  function getUserIPs(user) {
    if (!_.has(userToSockets, user)) return [];
    return userToSockets[user].map(function(sock) {
      console.log('sock', typeof sock);
      return sock.request.headers['x-real-ip'].split(':').pop();
    });
  }
  ...
  io.on('connection', function(socket) {
    ...
    socket.on('clip track', function(user, playlist, selectedIndex, startTime, endTime) {
...
module.exports = function(jub, chat, bot, config, io) {
  return new Socketeer(jub, chat, bot, config, io);
}
