...
function Chat(config, bot) {
  ...
  this.saveChatMsg = function(msgObj) {
    ...
    fs.appendFile(chat.cachePath(), line, function(err) {
      if (err) {
  ...
  function processMsg(msgObj, callback) {
    ...
    karma.parseForReport(msgObj.text, msgObj.user, function(user) {
      chat.jub.getKarma(user, function(sum) {
        chat.broadcastKarmaReport(user, sum, true);
      });
    });

    // Karma is being given
    karma.parseForGive(msgObj.text, msgObj.user, function(given) {
      chat.jub.giveKarmaAndGetTotal(given, function(sum) {
  ...
  this.welcome = function(user, callback) {
    bot.welcome(user, function(respObj) {
