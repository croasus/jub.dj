...
function DB(config, models, auth) {
  ...
  this.createAccount = function(username, password, email, callback, errCb) {
    ...
    auth.hashPassword(password, function(passwordHash) {
      db.isUsernameTaken(username, function(taken) {
        if (taken) {
  ...
  this.requestPasswordReset = function(username, callback, errCb) {
    ...
    models.UserPreferences.findOne({ name: username },
      ...
      if (!obj) {
        ...
        models.ResetPassword.findOne({ user: username },
                                   wrapTry(this, errCb, function(err, resetState) {
          var now = Date.now();
          auth.genPasswordResetToken(function(token) {
            if (resetState) {
              var minResetPeriod = 60000 * 10;
              // Make sure this request didn't come too soon after the last one
              if (resetState.lastRequestedAt + minResetPeriod > now.getTime()) {
                return callback(null,
                  'You may only reset your password once every 10 minutes.');
              }
              obj.token = token;
              lastRequestedAt = now;
            } else {
              obj = models.ResetPassword({
                user: username,
                token: token,
                lastRequestedAt: now
              });
  ...
  this.createPlaylist = function(user, playlistName, callback, errCb) {
    ...
    playlist.lastModifiedAt =  Date.now();
