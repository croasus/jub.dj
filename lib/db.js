...
function DB(config, models, auth) {
  ...
  this.getKarmaByDay = function(callback, errCb) {
    ...
    models.Karma.mapReduce({
      map: () => { emit(this.givenAt.getDay(), this.value) },
      reduce: (k, vs) => { return vs.reduce((a, b) => a + b); },
    }, wrapTry(this, errCb, (err, fetched) => {
      let map = _(fetched).map(x => [weekdays[x._id], x.value])
                          .fromPairs()
                          .value();
      callback(map);
    }));
  ...
  this.getKarmaReceived = function(callback, errCb) {
    ...
    models.Karma.mapReduce({
      ...
      let map = _(fetched).map(x => [x._id, x.value])
                          .fromPairs()
                          .value();
      callback(map);
    }));
  ...
  this.getKarmaBuddies = function(callback, errCb) {
    ...
    models.Karma.mapReduce({
      map: () => {
        let userPair = this.giver + "->" + this.recipient;
  ...
  this.getUserKarmaMap = function(user, callback, errCb) {
    if (typeof errCb !==  'function') { errCb = defaultErrCb; }
    models.Karma.mapReduce({
      map: () => { emit(this.type, this.value) },
      reduce: (k, vs) => { return vs.reduce((a,b) => a + b); },
      query: { recipient: user },
    }, (err, result) => {
      if (err) { return errCb(err); };
      let map = _(result).map(x => [x._id, x.value])
                         .fromPairs()
                         .value();
      callback(map);
    });
  ...
  this.drop = function(callback) {
    ...
    let series = _.chain(models)
                  .values()
                  .map(m => { return done => { m.remove({}, done); }; })
                  .value();
