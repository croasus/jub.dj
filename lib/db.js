...
function DB(config, models) {
  ...
  this.deletePlaylist = function(user, playlistName, callback) {
  ...
  this.updatePlaylistVideos = function(user, playlistName, updateFn) {
    if (!user || !playlistName)
      return;
    models.Playlist.findOne({ user: user, name: playlistName }, function(err, fetched) {
  ...
  this.updateOrCreatePlaylistMeta = function(user, playlistName, updateFn) {
    if (!user || !playlistName)
      return;
    models.Playlist.findOne({ user: user, name: playlistName }, function(err, fetched) {
  ...
  this.updateOrCreateUserPreferences = function(user, newPrefs, callback) {
    models.UserPreferences.findOne({ name: user }, function(err, obj) {
      ...
      //      additional places in the server. I's rather add it to ZERO.
  ...
  this.fetchOrInitUserPreferences = function(user, callback) {
    models.UserPreferences.findOne({ name: user }, function(err, res) {
      if (err) return console.error(err);
      if (!res) {
        console.log('no prefs; CREATING preferences');
        db.updateOrCreateUserPreferences(user, {}, callback);
      } else {
        callback(res);
      }
    });
  ...
  this.addKarma = function(karma, callback) {
  ...
  this.getUserKarmaMap = function(user, callback) {
    var mr = models.Karma.mapReduce({
      map: function() { emit(this.type, this.value) },
      reduce: function(k, vs) { return vs.reduce(function(a,x){return x+a}); },
  ...
  this.getUserKarmaTotal = function(user, callback) {
