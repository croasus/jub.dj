...
function DB(config, models, auth) {
  ...
  let RESET_PASSWORD_EXPIRATION = 3600 * 24 * 1000;

  mongoose.connect(config.mongodb_endpoint + '/' + config.mongodb_db, err => {
  ...
  this.deletePlaylist = function(user, playlistName, callback, errCb) {
    if (typeof errCb !==  'function') { errCb = defaultErrCb; }
    models.Playlist.remove({ user: user, name: playlistName}, err => {
  ...
  this.validateLogin = function(username, password, callback, errCb) {
    ...
    models.UserPreferences.findOne({ name: noCaseRegExp(username) },
                                   wrapTry(this, errCb, (err, fetched) => {
      if (err) { errCb(err); }
      if (fetched) {
        auth.checkPassword(password, fetched.passwordHash, (err, res) => {
  ...
  this.validateName = function(name, type) {
    ...
    if (!(/^[a-zA-Z0-9][a-zA-Z0-9-_]+$/.test(name))) {
      return type + ' must begin with a letter or number and can only contain'
  ...
  function validateNewAccount(username, password, email) {
    let nameError = db.validateName(username, 'Username');
    if (nameError) { return nameError; }
    let passwordError = db.validatePassword(password);
    if (passwordError) { return passwordError; }
    let emailError = db.validateEmailAddress(email);
    if (emailError) { return emailError; }
    return null;
  }

  this.validateNickname = function(nickname) {
    let nameError = db.validateName(nickname, 'Nickname');
  ...
  this.createAccount = function(username, password, email, callback, errCb) {
    ...
    let errorMsg = validateNewAccount(username, password, email);
    if (errorMsg) {
      return callback(errorMsg);
    }
    auth.hashPassword(password, passwordHash => {
      db.accountExists(username, taken => {
        if (taken) {
    ...
    models.UserPreferences.findOne({ name: noCaseRegExp(username) },
      ...
      if (!obj) {
        ...
        models.ResetPassword.findOne({ user: noCaseRegExp(username) },
                                   wrapTry(this, errCb, (err, resetState) => {
          let now = new Date();
          ...
          // Grant the request
          auth.genPasswordResetToken(token => {
            if (!resetState) {
            ...
            resetState.save((err, obj, numAffected) => {
              if (err) {
                errCb(err);
  ...
  function validatePasswordResetToken(token, fetched, callback) {
    ...
    let now = new Date();
    let expireTime = new Date(fetched.grantedAt.getTime() + RESET_PASSWORD_EXPIRATION);
    if (now > expireTime) {
  ...
  this.validatePasswordResetToken = function(username, token, callback, errCb) {
    ...
    models.ResetPassword.findOne({ user: noCaseRegExp(username) },
                                 wrapTry(this, errCb, (err, obj) => {
      if (err) { errCb(err); return callback(null, 'Unknown error.'); }
      validatePasswordResetToken(token, obj, (success, message) => {
  ...
  this.spendPasswordResetToken = function(username, token, callback, errCb) {
    ...
    models.ResetPassword.findOne({ user: noCaseRegExp(username) },
                                 wrapTry(this, errCb, (err, obj) => {
      if (err) { errCb(err); return callback(null, 'Unknown error.'); }
      validatePasswordResetToken(token, obj, (valid, message) => {
        if (!valid) {
  ...
  this.updatePlaylistVideos = function(user, playlistName, updateFn, errCb) {
    ...
    models.Playlist.findOne({ user: user, name: playlistName },
      ...
      if (typeof updateFn == 'function') {
        ...
        let processed = _.chain(fetched.videos)
          .sortBy(x => x.position)
          .map(x => _.omit(x.toObject(), ['position', '_id']))
          .value();

        // User modifies the videos and calls callback
        updateFn(processed, (post, finale) => {
          ...
          fetched.save((err, obj, numAffected) => {
            if (err) return errCb(err);
            finale();
          });
        });
  ...
  this.createPlaylist = function(user, playlistName, callback, errCb) {
    ...
    playlist.save((err, obj, numAffected) => {
      if (err) return errCb(err);
  ...
  this.updateUserPreferences = function(user, newPrefs, callback, errCb) {
    ...
    models.UserPreferences.findOne({ name: noCaseRegExp(user) },
      ...
      let updatableProps = ['color', 'requeueVideos', 'selectedPlaylist',
                            'showChatImages', 'showChatAnimations',
                            'addSpaceAfterMention', 'allowMuteStatus'];
      _.each(updatableProps, key => {
  ...
  this.addKarma = function(karma, callback, errCb) {
    if (typeof errCb !==  'function') { errCb = defaultErrCb; }
    let record = new models.Karma(karma);
  ...
  this.getKarmaByDay = function(callback, errCb) {
    ...
    models.Karma.mapReduce({
      ...
      var map = _(fetched).map(x => [weekdays[x._id], x.value])
                          .fromPairs()
                          .value();
      callback(map);
    }));
  ...
  this.getKarmaReceived = function(callback, errCb) {
    ...
    models.Karma.mapReduce({
      ...
      var map = _(fetched).map(function(x) {return [x._id, x.value];}) // TODO change back
                          .fromPairs()
                          .value();
      callback(map);
    }));
  ...
  this.getKarmaGiven = function(callback, errCb) {
    ...
    models.Karma.mapReduce({
      ...
      var map = _(fetched).map(x => [x._id, x.value])
                          .fromPairs()
                          .value();
      callback(map);
    }));
  ...
  this.getUserKarmaMap = function(user, callback, errCb) {
    ...
    models.Karma.mapReduce({
      ...
      if (err) { return errCb(err); };
      var map = _(result).map(x => { return [x._id, x.value]; })
                         .fromPairs()
                         .value();
      callback(map);
    });
  ...
  this.drop = function(callback) {
    ...
    let series = _.chain(models)
                  .values()
                  .map(m => { return function(done) { m.remove({}, done); }; })
                  .value();

      // After all the steps are done, call `callback`
      series.push(done => { callback(); done() });
      async.series(series);
  }
